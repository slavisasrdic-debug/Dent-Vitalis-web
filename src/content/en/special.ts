...
if (pairs.length !== captions.length)
  throw new Error('English gallery must retain all 15 source photo pairs');
      ...
      group([
        {
          ...
          before: {
            ...pairs[i]!.before,
            alt: `${text(captions[i]!)} — before`,
          },
          after: { ...pairs[i]!.after, alt: `${text(captions[i]!)} — after` },
          beforeLabel: 'Before',
          afterLabel: 'After',
        },
...
export const specialPages: Special[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      group([heading(text('p264')), ...row(26, 0, '1 2')], 'contact-card'),
      group(
        [
          heading(text('p268')),
          ...row(27, 0, '0 1 2'),
      ...
      group([heading(text('p278')), ...row(29, 0, '1')], 'contact-card'),
    ],
  },
];
