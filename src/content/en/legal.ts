...
import { englishLegalRoutes, legalLinks } from './legal-routes';
import type { ContentBlock, InnerPage } from '../inner-pages';
import { plain } from '../seo';
import { alt } from './assets';

// User approved the existing public English legal articles, including their
// structure and links. The original HTML and its SHA-256 remain immutable.
export const englishLegalPages: InnerPage[] = publicLegal.map((source) => {
  ...
  return {
    route: source.route,
    referenceRoute:
      source.route === englishLegalRoutes.privacy
