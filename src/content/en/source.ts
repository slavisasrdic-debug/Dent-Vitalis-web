import catalogue from '../../../data/translations/en-source.json';
...
export function text(id: string): string {
  ...
  // Preserve English numeric punctuation and an unbreakable currency space.
  value = value.replace(
    /€\s*(\d(?:[\d,.]*\d)?)/g,
    (_, amount: string) =>
      `${amount.replaceAll(',', '_').replaceAll('.', ',').replaceAll('_', '.')} €`,
  );
...
export function row(
  ...
  return selection
    ...
    .map((part) => {
      ...
      if (!match) throw new Error(`Invalid English selection: ${part}`);
