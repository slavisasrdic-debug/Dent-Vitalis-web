...
import { germanLegalRoutes, legalLinks } from './legal-routes';
import type { ContentBlock, InnerPage } from '../inner-pages';
import { plain } from '../seo';
import { alt } from './assets';

// User approved the existing public German legal articles, including their
// structure and links. The original HTML and its SHA-256 remain immutable.
export const germanLegalPages: InnerPage[] = publicLegal.map((source) => {
  ...
  return {
    route: source.route,
    referenceRoute:
      source.route === germanLegalRoutes.privacy
