...
function add(
  ...
  page.blocks = id === 'directions' ? labelParkingLinks(blocks, 'de') : blocks;
  pages.push(page);
  return page;
}
...
const denture = add('four-implant-denture', t(2, 0, 2), t(2, 0, 4), [
  ...row(2, 0, '19 21 h:23 b:25 b:26 b:27 b:28 b:29 31'),
  ...row(2, 1, 'h:0'),
...
const bridge = add('fixed-implant-bridge', t(1, 3, 3), t(3, 0, 4), [
  ...
  ...row(3, 1, 'h:0'),
  ...['h3:2 3', 'h3:5 6', 'h3:8 9', 'h3:11 12', 'h3:13 14', 'h3:15 16'].map(
    (s) => section(3, 1, s),
  ),
]);
...
add('materials', label('materials'), text('p46'), [
...
add('new-implants', label('new-implants'), text('p49').split('\n')[1]!, [
...
add('sedation', label('sedation'), text('p55'), [
  ...row(12, 0, '0'),
...
prices.hero.photo = {
  image: 'DV-cjenik-de.webp',
  alt: alt('DV-cjenik-de.webp'),
};
...
const faqPage = add('faq', label('faq'), text('p199'), [
  { ...h('p202'), id: 'zahnmedizin' },
  ...questions.slice(0, 3),
  { ...h('p209'), id: 'dentvitalis' },
  ...questions.slice(3),
]);
faqPage.sidebar = {
  type: 'navigation',
  ...
  position: 'before',
  items: [
    ['zahnmedizin', text('p202')],
    ['dentvitalis', text('p209')],
  ].map(([id, label]) => ({
    label: label!,
    href: '#' + id,
    icon: false,
...
if (!quoted) throw new Error('Missing German Ana research title');
