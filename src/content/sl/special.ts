...
if (pairs.length !== captions.length)
  throw new Error('Slovenian gallery must retain all 15 source photo pairs');
      ...
      group([
        {
          ...
          before: {
            ...pairs[i]!.before,
            alt: `${text(captions[i]!)} — pred`,
          },
          after: { ...pairs[i]!.after, alt: `${text(captions[i]!)} — po` },
          beforeLabel: 'Pred',
          afterLabel: 'Po',
        },
...
export const specialPages: Special[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      group([heading(text('p267')), ...row(26, 0, '1 2')], 'contact-card'),
      group(
        [
          heading(text('p271')),
          ...row(27, 0, '0 1 2'),
      ...
      group([heading(text('p281')), ...row(29, 0, '1')], 'contact-card'),
    ],
  },
];
