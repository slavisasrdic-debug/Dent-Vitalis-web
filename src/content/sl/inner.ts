...
function add(
  ...
  page.blocks = id === 'directions' ? labelParkingLinks(blocks, 'sl') : blocks;
  pages.push(page);
  return page;
}
...
const whitening = add('whitening', t(4, 0, 3), t(4, 0, 5), [
  ...row(4, 0, '16 18 20 22'),
  ...row(4, 1, 'h:0'),
  ...['h3:1 2 4', 'h3:6 7', 'h3:9 10', 'h3:12 13'].map((s) => section(4, 1, s)),
...
add('new-implants', label('new-implants'), text('p51'), [
...
add('sedation', label('sedation'), text('p57'), [
  ...row(12, 0, '0'),
...
add('payment', label('payment'), t(15, 0, 1), [
  ...
  section(15, 4, 'h:0 2 4 6 8 10 12'),
  { type: 'payment-code' },
]);
...
prices.hero.photo = {
  image: 'DV-cjenik-sl.webp',
  alt: alt('DV-cjenik-sl.webp'),
};
...
if (!quoted) throw new Error('Missing Slovenian Ana research title');
