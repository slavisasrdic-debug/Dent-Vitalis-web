import catalogue from '../../../data/translations/sl-source.json';
...
export function text(id: string): string {
  ...
  if (id === 't5.r0.c0.p10') value = 'Zobne krone že od 220 €';
  value = value.replace(/\b(\d)(\d{3})(?=\s*€)/g, '$1.$2');
...
export function row(
  ...
  return selection
    ...
    .map((part) => {
      ...
      if (!match) throw new Error(`Invalid Slovenian selection: ${part}`);
