...
HR navigacija koristi kratke izvorne fragmente „Informacije”, „Iskustva” i „FAQ”, dok pune naslove stranica i odlomke ne skraćuje. Time se izbjegava prekrivanje jezičnog izbornika kontaktnim gumbom na 992px bez promjene referentnih fontova, paddinga ili media uvjeta. Provjera obuhvaća stvarni `summary` hitbox i `scrollWidth` djece flex retka: sama širina roditeljskog containera može sakriti preklapanje, što je potvrđeno u WebKitu.

Korisnički dodatak za HR logotip (8. rujna): `hr/site.ts` sadrži odobreni natpis **ORDINACIJE DENTALNE MEDICINE** umjesto „Dentvitalis Rijeka”. `Header` ga preko postojeće `tagline` vrijednosti prikazuje u `brand-hr` varijanti: Montserrat 500, 8.2px desktop / 7.75px do 991px, tracking 0 i line-height 1.4. Posljednji redak poravnat je na širinu nepromijenjenog logotipa (150/140px), bez JS mjerenja, skaliranja slike ili prijeloma u dva reda. Font je izričito vezan uz `--font-body` i na directory/legal stranicama. IT natpis i stil ostaju nepromijenjeni. Regresija: `tests/header-tagline.spec.ts` u Chromiumu/WebKitu.
