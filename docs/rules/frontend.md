...
- Interna sidra koriste zajednički `html` scroll-padding: visina headera + stvarna visina sticky breadcrumb trake + `--anchor-gap`. `initializeScrollOffsets` promatra samo promjenu visine breadcrumb trake; ne mijenja geometriju ni native klik/hash/povijest. Ne dodavati drugi header offset kroz lokalni scroll-margin. Provjeriti klik, direktan fragment/refresh, tipkovnicu i no-JS na oba jezika, uključujući prelomljeni breadcrumb.
