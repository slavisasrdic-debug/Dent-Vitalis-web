...
const documents = JSON.parse(
  readFileSync('src/content/sl/legal-public.json', 'utf8'),
) as typeof import('../src/content/sl/legal-public.json');
for (const article of documents)
  test(`Slovenian legal source text, lists, emphasis and links: ${article.route}`, async ({
    page,
    request,
  }) => {
    ...
    const result = await page.evaluate(
      ({ source, rendered }) => {
        ...
        const normalizedHref = (href: string) =>
          href
            .replace(/^https:\/\/www.dentvitalis.com(?=\/si)/, '')
    ...
    if (article.route.includes('zasebnosti')) {
      // Slovenian source has eleven .question headings plus one .policy-title.
      expect(result.headings.filter((h) => h.tag === 'H2')).toHaveLength(12);
      expect(result.headings.filter((h) => h.tag === 'H3')).toHaveLength(9);
      expect(result.headings.filter((h) => h.tag === 'H4')).toHaveLength(8);
    } else
      expect(result.headings.map((h) => h.text)).toContain(
