...
const documents = JSON.parse(
  readFileSync('src/content/de/legal-public.json', 'utf8'),
) as typeof import('../src/content/de/legal-public.json');
for (const article of documents)
  test(`German legal source text, lists, emphasis and links: ${article.route}`, async ({
    page,
    request,
  }) => {
    ...
    const result = await page.evaluate(
      ({ source, rendered }) => {
        ...
        const normalizedHref = (href: string) =>
          href
            .replace(/^https:\/\/www.dentvitalis.com(?=\/de)/, '')
    ...
    if (article.route.includes('datenschutz')) {
